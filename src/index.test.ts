...
describe('index.test.ts', () => {
    ...
    it('should call populateVoiceList on window load', () => {
        expect((API.populateVoiceList as any).mock.calls.length).toBe(1)
    })
    it('should set a new voice default', () => {
        API.selectVoice(voices[0] as SpeechSynthesisVoice)
    ...
    it('should return error on try to call speak without voice and default voice selected', () => {
        ...
        expect(() => API.speak('this is an example', null)).toThrowError('voice is required')
    })
    ...
    it('should add listen to start and end speak event', () => {
        ...
        expect((API.utterance.addEventListener as any).mock.calls.length).toBe(2)
    })
})
